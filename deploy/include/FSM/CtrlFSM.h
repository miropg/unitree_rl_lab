...
public:
    ...
    // Force a transition to the given state id, bypassing joystick-based
    // transition checks. Used for scripted/autopilot testing (e.g. no
    // physical remote available). Thread-safe with respect to run_().
    void forceState(int stateId)
    {
        std::lock_guard<std::mutex> lock(state_mutex_);
        if(currentState && currentState->isState(stateId))
        {
            return;
        }
        for(auto & state : states)
        {
            if(state->isState(stateId))
            {
                spdlog::info("FSM: Force change state from {} to {}", currentState->getStateString(), state->getStateString());
        ...
        spdlog::warn("FSM: forceState() could not find state id {}", stateId);
    }
    ~CtrlFSM()
    {
        states.clear();
