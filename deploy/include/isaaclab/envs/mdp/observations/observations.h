...
REGISTER_OBSERVATION(velocity_commands)
{
    ...
    if(isaaclab::autopilot::active.load())
    {
        obs[0] = std::clamp(isaaclab::autopilot::lin_vel_x.load(), cfg["lin_vel_x"][0].as<float>(), cfg["lin_vel_x"][1].as<float>());
        obs[1] = std::clamp(isaaclab::autopilot::lin_vel_y.load(), cfg["lin_vel_y"][0].as<float>(), cfg["lin_vel_y"][1].as<float>());
        obs[2] = std::clamp(isaaclab::autopilot::ang_vel_z.load(), cfg["ang_vel_z"][0].as<float>(), cfg["ang_vel_z"][1].as<float>());
        return obs;
    }
